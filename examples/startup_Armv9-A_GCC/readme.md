# Armv9-A SMP AArch64 Startup Code for GCC for bare-metal targets

This is a multi-core (SMP) startup code example for AArch64, written in C and assembler, to demonstrate support for building and debugging Armv9-A bare-metal applications with GCC for bare-metal targets and the Debugger.
...
Navigate to the `...\startup_Armv9-A_GCC` directory, and type:
...
`aarch64-none-elf-gcc -c -march=armv9-a -g -O1`

## Running the example

```text
FVP_Base_RevC-2xAEMvA -C cluster0.NUM_CORES=4 -C cluster1.NUM_CORES=0 -C bp.secure_memory=0 -C cache_state_modelled=0 -C pctl.startup=0.*.*.* -C cluster0.has_arm_v9-0=1 -a startup_Armv9-A_GCC.axf
```
...
2. Select `startup_Armv9-A_GCC_FVP` from the list of **Generic Arm C/C++ Application** configurations.
