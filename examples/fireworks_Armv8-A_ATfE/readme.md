# Fireworks bare-metal Armv8-A SMP example for Arm Toolchain for Embedded

Fireworks example to demonstrate support for building and debugging Armv8-A SMP bare-metal applications with Arm Toolchain for Embedded and the Debugger.
...
Navigate to the `...\fireworks_Armv8-A_ATfE` directory, and type:
...
`clang -c --target=aarch64-none-elf -march=armv8-a -g -O1 -D SMP`
...
```text
FVP_Base_RevC-2xAEMvA -C cluster0.NUM_CORES=4 -C cluster1.NUM_CORES=0 -C bp.secure_memory=0 -C cache_state_modelled=0 -C pctl.startup=0.*.*.* -a fireworks_Armv8-A_ATfE.axf
```
...
2. Select `fireworks_Armv8-A_ATfE` from the list of **Generic Arm C/C++ Application** configurations.
