# Armv8-A SMP AArch64 Startup Code for GCC for bare-metal targets

This is a multi-core (SMP) startup code example for AArch64, written in C and assembler, to demonstrate support for building and debugging Armv8-A bare-metal applications with GCC for bare-metal targets and the Debugger.

## Purpose and scope

This is a multi-core (SMP) startup code example for AArch64, written in C and assembler, to demonstrate support for building and debugging Armv8-A bare-metal applications with GCC for bare-metal targets and the Debugger.
It includes a vector table, reset handler, cache and MMU configuration, interrupt controller (GICv3) initialization, NEON support, and is illustrated by a semihosted SMP prime number generator example application. It illustrates Debugger support for Armv8-A, AArch64, SMP, Exception Level changing, and semihosting.

The executable is intended for running on an Armv8-A SMP FVP model such as the RevC-2xAEMvA FVP model.
...
Navigate to the `...\startup_Armv8-A_GCC` directory, and type:
...
`aarch64-none-elf-gcc -c -march=armv8-a -g -O1`

## Running the example

```text
FVP_Base_RevC-2xAEMvA -C cluster0.NUM_CORES=4 -C cluster1.NUM_CORES=0 -C bp.secure_memory=0 -C cache_state_modelled=0 -C pctl.startup=0.*.*.* -a startup_Armv8-A_GCC.axf
```
...
2. Select `startup_Armv8-A_GCC_FVP` from the list of **Generic Arm C/C++ Application** configurations.
