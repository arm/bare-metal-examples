# Fireworks bare-metal Armv9-A SMP example for GCC for bare-metal targets

Fireworks example to demonstrate support for building and debugging Armv9-A SMP bare-metal applications with GCC for bare-metal targets and the Debugger.

## Purpose and scope

This example demonstrates the support for building and debugging Armv9-A SMP bare-metal applications with GCC for bare-metal targets and the Debugger. Fireworks is a full-featured bare-metal example written in C and assembler that produces a graphical simulated fireworks display and flying aeroplane banner on the screen.

This bare-metal example is targeted at an Armv9-A SMP FVP model, complete with startup code (vector table, reset handler, interrupt handler, cache and MMU configuration, interrupt controller (GICv3) and timer initialization, NEON support), LCD screen driver, retargeted printf() output to a UART, etc. The code is built for AArch64 only and illustrates Debugger support for Exception Level changing.
...
This example can be built with GCC for bare-metal targets on the command-line with the supplied `makefile` using the `make` utility.

Navigate to the `...\fireworks_Armv9-A_GCC` directory, and type:
...
`aarch64-none-elf-gcc -c -march=armv9-a -g -O1 -D SMP -D USE_LSE_ATOMIC -D USE_SERIAL_PORT -D STANDALONE`

`-D SMP` is used to switch the fireworks application code into SMP mode. `USE_SERIAL_PORT` and `STANDALONE` are used to select retargeting (see below).

`USE_LSE_ATOMIC` selects the use of Armv8.1-A FEAT_LSE Large System Extension (LSE) atomics.

## Running the example

```text
FVP_Base_RevC-2xAEMvA -C cluster0.NUM_CORES=4 -C cluster1.NUM_CORES=0 -C bp.secure_memory=0 -C cache_state_modelled=0 -C pctl.startup=0.*.*.* -C cluster0.has_arm_v9-0=1 -C cluster1.has_arm_v9-0=1 -a fireworks_Armv9-A_GCC.axf
```

## Example code structure

### Source Files in ...\src and ...\asm directories

- `startup.S` : assembler boot code reset handler, interrupt handler, cache and MMU config, NEON enable.
- `vectors.S` : Vector tables.
- `v8_aarch64.S` : assembler code for Cache and ID register functions.
- `MP_Mutexes.S` : assembler code for Mutex functions.
- `GICv3_*.c` : Generic driver code for GICv3 (re)distributor.
- `main.c` : main() code that launches the fireworks application
- `Fireworks.c` : the main fireworks application
- `timer_interrupts.c` : timer and interrupts initialization and handling
- `screen.c` : a driver for FVP LCD screen
- `retarget.c` : functions retarget all semihosting usage, including redirecting printf() output to the UART
- `uart.c` : a simple polled UART driver
- `banner_data.c` : Aeroplane banner (120x600) pixel array

### Description of the Code Execution

The first stage of boot is the reset handler, where the stack pointers, MMU, page tables and caches are initialized. Then the C library and the C run-time environment are initialised, before calling main().

The main fireworks application is launched from `main()`. `main()` first enables the caches, clears the screen, initializes the timer, enables the IRQ input to CPU0, then calls `fireworks()`.

The fireworks application fires "sparks" as fast as it can, and also displays and scrolls the banner. During the fireworks display, CPU0's private timer is also ticking, generating interrupts. On each interrupt, the LEDs are advanced one position.
...
2. Select `fireworks_Armv9-A_GCC` from the list of **Generic Arm C/C++ Application** configurations.
...
13. In the **Command** field, enter `break EL1N:MainApp`, and continue execution (press F8). Within `MainApp`, CPU0 is used to produce a graphical simulated fireworks display, CPU1 is used to move the flying aeroplane banner on the screen, and the other CPUs go back to sleep.
14. In the **Breakpoints** view, delete all breakpoints and continue execution (press F8). A simulated fireworks display will be displayed on the model's LCD screen.
...
4. In the **Commands** view, enter `mmu print`. This gives a similar output to the above:
   ...
   ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
   + 0x00000000          | TTBR0_EL1      | NP:0x00000000800F0000 |                       | TBI1=0, TBI0=0, AS=0, IPS=4GB, TG1=4KB, SH1=0x0, ORGN1=0x0, IRGN1=0x0, EPD1=1, A1=0, T1SZ=0, TG0=4KB, SH0=0x3, ORGN0=0x1, IRGN0=0x1, EPD0=0, T0SZ=32, ASID=0
    + 0x00000000         | Level 1 Table  | NP:0x00000000800F2000 |                       | APTable=0x0, UXNTable=0, PXNTable=0
    ...
    - 0x40000000         | Invalid        |                       |                       |
    + 0x80000000         | Level 1 Table  | NP:0x00000000800F1000 |                       | APTable=0x0, UXNTable=0, PXNTable=0
...
1. Set a breakpoint on the `fireworks(id)` function and run to it (press F8).
2. Select Core 0 in the **Debug Control** view.
3. In the **Breakpoints** view, click on the **Manage Signals...** button, select **Stop** and **Print** for **EL1_IRQ_SPx_NON-SECURE**, then **OK**. This is equivalent to typing `handle EL1_IRQ_SPx_NON-SECURE stop` on the CLI.
4. Continue execution by pressing the green run/continue button (press F8). The fireworks display will start, and the IRQ breakpoint will be hit shortly after, due to the timer interrupt occurring.
5. Restart execution again (press F8). The LEDs will advance, and the fireworks display runs for another short period until the timer interrupt occurs again.
6. Single-step (press F5) through the `IRQ_Handler` and observe the AAPCS-corruptible registers being pushed onto the stack \[AAPCS=Procedure Call Standard for the Arm Architecture\], and step again to see C code being called from assembler, continue stepping until `irqHandler()`.
7. `irqHandler()` clears the timer interrupt and advances the LED sequence. Step through this C code in `irqHandler()`, and back out through the assembler code of `IRQ_Handler` that pops the AAPCS-corruptible registers off the stack, and finally returns from the interrupt.
