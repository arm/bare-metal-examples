# Fireworks bare-metal Armv9-A SMP example for Arm Toolchain for Embedded

Fireworks example to demonstrate support for building and debugging Armv9-A SMP bare-metal applications with Arm Toolchain for Embedded and the Debugger.

## Purpose and scope

This example demonstrates the support for building and debugging Armv9-A SMP bare-metal applications with Arm Toolchain for Embedded and the Debugger. Fireworks is a full-featured bare-metal example written in C and assembler that produces a graphical simulated fireworks display and flying aeroplane banner on the screen.
...
Navigate to the `...\fireworks_Armv9-A_ATfE` directory, and type:
...
`clang -c --target=aarch64-none-elf -march=armv9-a -g -O1 -D SMP -D USE_LSE_ATOMIC`

`-D SMP` is used to switch the fireworks application code into SMP mode.
...
```text
FVP_Base_RevC-2xAEMvA -C cluster0.NUM_CORES=4 -C cluster1.NUM_CORES=0 -C bp.secure_memory=0 -C cache_state_modelled=0 -C pctl.startup=0.*.*.* -C cluster0.has_arm_v9-0=1 -C cluster1.has_arm_v9-0=1 -a fireworks_Armv9-A_ATfE.axf
```
...
2. Select `fireworks_Armv9-A_ATfE` from the list of **Generic Arm C/C++ Application** configurations.
