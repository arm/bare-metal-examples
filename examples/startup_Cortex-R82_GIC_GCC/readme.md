# Cortex-R82 Startup Example Code with GIC for GCC for bare-metal targets
...
This example uses HLT-based semihosting, selected at build-time by linking with `--specs=rdimon.specs`.
...
Navigate to the `...\startup_Cortex-R82_GIC_GCC` directory, and type:
...
```text
FVP_BaseR_AEMv8R -C cluster0.NUM_CORES=1 -C cluster0.has_aarch64=1 -C cluster0.gicv3.cpuintf-mmap-access-level=2 -C cluster0.gicv3.SRE-enable-action-on-mmap=2 -C cluster0.gicv3.SRE-EL2-enable-RAO=1 -C cluster0.gicv3.extended-interrupt-range-support=1 -C cluster0.cpu0.semihosting-stack_base=0 -a startup_Cortex-R82_GIC_GCC.axf
```
...
```text
Cortex-R82 bare-metal startup example with GIC, using GCC for bare-metal targets
...
2. Select `startup_Cortex-R82_GIC_GCC-FVP` from the list of **Generic Arm C/C++ Application** configurations.
...
10. Finally, code execution branches to `_start` to setup the C library run-time environment. In the **Commands** view, set a breakpoint on `main()` with `break main`, and run to it (press F8). `main()` is reached in main.c.
