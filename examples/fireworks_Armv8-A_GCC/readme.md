...
Navigate to the `...\fireworks_Armv8-A_GCC` directory, and type:
...
`aarch64-none-elf-gcc -c -march=armv8-a -g -O1 -D USE_SERIAL_PORT -D STANDALONE`
...
```text
FVP_Base_RevC-2xAEMvA -C cluster0.NUM_CORES=4 -C cluster1.NUM_CORES=0 -C bp.secure_memory=0 -C cache_state_modelled=0 -C pctl.startup=0.*.*.* -a fireworks_Armv8-A_GCC.axf
```
...
2. Select `fireworks_Armv8-A_GCC` from the list of **Generic Arm C/C++ Application** configurations.
